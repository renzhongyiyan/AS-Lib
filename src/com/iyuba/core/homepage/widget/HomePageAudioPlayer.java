...
//		isvip = ConfigManager.Instance().loadInt("isvip") == 1 ? true : false;
	...
	private void playVideo() {// finish是否从头开始
		...
		if (source == 0) {
			if (SettingConfig.Instance().isHighSpeed()) {
				append = "http://staticvip.iyuba.com/sounds/voa";
			} else {
				append = "http://static.iyuba.com/sounds/voa";
			}
			url = append + newsArrayList.get(listPosition).musicUrl;
		} else if (source == 1) {
			if (SettingConfig.Instance().isHighSpeed()) {
				append = "http://staticvip.iyuba.com/sounds/minutes/";
			} else {
				append = "http://static.iyuba.com/sounds/minutes/";
			}
			url = append + newsArrayList.get(listPosition).musicUrl;
		} else if (source == 2) {
			if (SettingConfig.Instance().isHighSpeed()) {
				append = "http://staticvip.iyuba.com/sounds/song/";
			} else {
				append = "http://static.iyuba.com/sounds/song/";
			}
			url = append + newsArrayList.get(listPosition).musicUrl;
		}
	...
	private String getDetailUrl() {
		String url = null;
		if (source == 1) {
			url = "http://apps.iyuba.com/minutes/textApi.jsp?bbcid="
					+ newsArrayList.get(listPosition).id;
		} else if (source == 0) {
			url = "http://apps.iyuba.com/voa/textNewApi.jsp?voaid="
					+ newsArrayList.get(listPosition).id;
		} else if (source == 2) {
			url = "http://apps.iyuba.com/afterclass/getText.jsp?SongId="
					+ newsArrayList.get(listPosition).id;
		}
		return url;
	}
